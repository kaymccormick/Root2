...
namespace KayMcCormick.Dev.TestLib.Fixtures
{
    ...
    public class LoggingFixture : IDisposable
    {
        ...
        public void SetOutputHelper ( ITestOutputHelper value, object testClass = null, NLog.LogLevel minLogLevel = null)
        {
            ...
            var att = type?.GetCustomAttributes ( typeof ( LoggingAttribute ), true ) ?? Array.Empty<Attribute>() ;
            ...
            bool fUpdated = false; 
            LoggingAttributeContext context = new LoggingAttributeContext();
            context.Target = _xunitTarget ;
            foreach (LoggingAttribute o in att)
            {
                if ( o.Apply ( context ) ) fUpdated = true ;
            }

            
            if ( testClass != null ) {
                var rules = testClass.GetType().GetCustomAttributes(typeof(LoggingRuleAttribute), true);
                foreach ( var rule in rules )
                {
                    if(rule is LoggingRuleAttribute la)
                    {
                        LoggingRule rule2 = new LoggingRule ( la.LoggerNamePattern , la.Level , _xunitTarget ) ;
